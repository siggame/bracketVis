...
class BracketViewer
    ...
    drawMatchPageFunc: (id, match) =>
        ...
        startGame.pos.w = 120
        startGame.pos.h = 80
        startGame.pos.x = (0.5 * @canvas.width) - (startGame.pos.w/2)
        startGame.pos.y = (0.7 * @canvas.height) - (startGame.pos.h/2)
        startGame.text = "Play Game"
        startGame.color = "#AAFFAA"
        startGame.func = () =>
             window.open('http://vis.megaminerai.com/#/?logUrl='+
                  encodeURIComponent(match.log_location),'_blank')
    ...
    createBrackets: () ->
        ...
        ###
         # Iterate throught the entire body of the first bracket and find
         # all cases where a buy is involved. Replace it with a player slot
         # of the player who was not the buy
        ###

        for id,match of @bracket1.matches
            if match.player_1 == "bye" or match.player_2 == "bye"
                w = if match.player_1 == "bye" then match.player_2 else match.player_1
                @bracket1.matches[id] = new PlayerSlot(w)
                @bracket1.matches[id].visible = true

        for id,match of @bracket1.matches
            if match.previous_matches? and match.previous_matches.length > 0
                prevMatch1id = match.previous_matches[0].toString()
                prevMatch2id = match.previous_matches[1].toString()
                prevMatch1 = @bracket1.matches[prevMatch1id]
    ...
    drawMatch : (bracket, match, x1, x2, y1, y2, xinc, goLeft, alignment) =>
        ...
        ###
        first draw this match bar. It is located center in the middle of the
        rectangle passed into the recursive function between x1, x2, y1, y2
        ###
        if match.isVisible()
            if match.is_slot()
                text = match.player
        ...
        ###
        Now we determine the position of the next matches/playerslots. We first
        check if it's the case that the next two are player slots and do not
        have any previous matches. These will only occur as the leaves of the
        first tournament since even player slots in the second bracket
        have losses that occured in matches in the first bracket
        ###
        if(match.previous_matches.length == 0)
            # split the y in half between. This will be done recursively
            slot1y1 = y1
            slot1y2 = y1 + (height/2)
            slot2y1 = slot1y2
            slot2y2 = y2
